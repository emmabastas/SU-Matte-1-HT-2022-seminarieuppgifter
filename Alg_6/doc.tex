...
\begin{enumerate}
        \item[a)] Vektorn $\overline{u}_{1}$ har kordinaterna $(5, -2)$ i basen $\ee, \eee$. Bestämm $\uu$:s koordinater på avseende på basen $\ff, \fff$.
...
\noindent Vi tolkar detta geometriskt, en regelbunden hexagon kan konstrueras med sex liksidiga trianglar:
...
\begin{tikzpicture}[scale = 3]
  ...
  \draw[->, very thick, dashed] (A)--(B) node[pos=0.4, above] {$\ee$};
  \draw[->, very thick, dashed] (A)--(C) node[pos=0.4, above] {$\ff$};
  \draw[->, very thick, dashed] (A)--(D) node[pos=0.4, above left] {$\eee$};
